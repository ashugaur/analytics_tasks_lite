...
def convert_markdown_to_html_crude(
    markdown_file,
    output_file="convert_markdown_to_html_crude.html",
    page_title="",
    navigation_title="",
    page_subtitle=None,
    ...
    """
    Bugfix: Bottom rows showing for blank lines...
    ...
    def parse_markdown(md_content, base_path):
        ...
        while i < len(lines):
            ...
            # Regular text (only add if line is not empty and we're in a section)
            elif line.strip() and current_section:
                target = current_subsection if current_subsection else current_section
                if "content" not in target:
                    target["content"] = []

                # Check if last item was text, if so append to it
                if target["content"] and target["content"][-1].get("type") == "text":
                    target["content"][-1]["content"] += "\n" + line
                else:
                    target["content"].append({"type": "text", "content": line})

            # Skip blank lines - don't create text blocks for them
            elif not line.strip():
                i += 1
                continue
    ...
    def process_content(content_items, section_id):
        ...
        for item in content_items:
            ...
            if content_type == "text":
                ...
                html += f'                    <div class="reference-block">{item["content"]}</div>\n'

            elif content_type == "code":
                block_id = f"{section_id}-code-{code_counter}"
                code_counter += 1
                collapsed_class = "collapsed" if code_blocks_collapsed else ""
                toggle_text = "Expand" if code_blocks_collapsed else "Collapse"
                html += f'''                    <div class="code-container">
                        <div class="code-header">
                            <span class="code-language">{item["language"]}</span>
                            <div class="code-actions">
                                <button class="code-btn" onclick="toggleCode('{block_id}')">
                                    <span id="{block_id}-toggle">{toggle_text}</span>
                                </button>
                                <button class="code-btn" onclick="copyCode('{block_id}')">Copy</button>
                            </div>
                        </div>
...
"""
        ...
        for section in sections:
            ...
            for subsection in section.get("subsections", []):
                subsection_id = f"{section['id']}-{subsection['title'].lower().replace(' ', '-').replace(':', '').replace('(', '').replace(')', '').replace('/', '-')}"
...
"""
    ...
    <script>
        ...
        function toggleCode(blockId) {
            ...
            const toggleText = document.getElementById(blockId + '-toggle');
    ...
    try:
        ...
        print(f"📝 Reference blocks: {total_refs}")
...
if __name__ == "__main__":
    convert_markdown_to_html_crude(
        ...
        output_file=r"C:\my_disk\projects\visual_library\____uat\diagrams.html",
        ...
        default_theme="dark",
        navigation_title="📑 Diagram Navigation",
        include_code_blocks=True,
