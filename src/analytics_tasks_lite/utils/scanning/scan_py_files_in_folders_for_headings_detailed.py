...
def scan_py_files_in_folders_for_headings_detailed(
    unc,
    md_name="Project report",
    heading_order=0,
    start_heading=1,
    max_heading_level=6,
    ...
    def scan_py_files_for_headings_modified(unc, num_hashes):
        ...
        if heading_order == 0:
            pattern0 = re.compile(
                r"^\s*(#|--) "
            )  # Matches lines with leading spaces followed by # or --
            pattern1 = re.compile(
                r"^\s*# %% "
            )  # Matches lines with # %% and leading spaces
            pattern2 = re.compile(
                r"^\s*###### "
            )  # Matches lines with ###### and leading spaces
            pattern3 = re.compile(
            ...
            pattern7 = re.compile(
                r"^\s*#([^ ]+)"
            )  # Matches lines with # followed by non-space characters
        ...
        df["hierarchy"] = np.where(
            (df["flag"].between(1, max_heading_level)),
            df["flag"].apply(lambda x: "#" * x),
            np.nan,
        )
