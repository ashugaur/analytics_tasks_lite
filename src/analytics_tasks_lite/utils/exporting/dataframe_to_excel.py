...
def dataframe_to_excel(
    ...
    Known issues:
    - Data should not have nan values, replace with '' where applicable.
    ...
    if file_exists:
        ...
        for row_num, (idx_val, row) in enumerate(df.iterrows(), start=header_row + 1):
            ...
            for col_num, (col_name, value) in enumerate(
                zip(df.columns, row), start=start_col
            ):
