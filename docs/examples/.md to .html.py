...
def convert_markdown_to_html(
    ...
    def embed_html_chart(html_path, height, chart_index):
        ...
        try:
            ...
            if has_theme:
                theme_registration = (
                    f"\n        // Register theme function name with report (looked up at call time)\n"
                    f"        if (!window._chartThemeFnNames) window._chartThemeFnNames = [];\n"
                    f"        window._chartThemeFnNames.push('setChartTheme_{chart_index}');\n"
                )
    ...
    def embed_datatable(data_path, kwargs_str, table_index):
        ...
        try:
            ...
            # Build per-column format lookup for JS (from format_file config)
            _col_formats_js = {}
            for col in df.columns:
                ...
                if _fmt:
                    _col_formats_js[col_lower] = _fmt            
...
window.dtColMeta_{query_id} = {_json.dumps({str(c): col_meta.get(str(c), {}) for c in df.columns})};
window.dtColFormats_{query_id} = {_json.dumps(_col_formats_js)};
window.dtFreezeSet_{query_id} = {_json.dumps(list(freeze_set))};
</script>
...
"""
    ...
    <script>
        ...
        function toggleTheme() {
            ...
            (window._chartThemeFnNames || []).forEach(name => { try { if (window[name]) window[name](isDark); } catch(e) {} });
        }
        ...
        // Sync all embedded charts to the active theme on page load
        ...
        function dtBuildRow(rowData, cols, idx, hidden) {
            ...
            // Find the queryId from the tbody's id
            var qid = tr._dtQueryId || dtBuildRow._lastQid || '';
            var colMeta = window['dtColMeta_' + qid] || {};
            var colFormats = window['dtColFormats_' + qid] || {};
            var freezeSet = window['dtFreezeSet_' + qid] || [];
            cols.forEach(function(c) {
                var td = document.createElement('td');
                var cLower = c.toLowerCase();
                td.setAttribute('data-col', cLower);
                var raw = rowData[c];
                var display;
                if (raw === null || raw === undefined) {
                    display = '';
                } else {
                    // Check format_file formats first
                    var fmt = colFormats[cLower];
                    if (fmt && typeof raw === 'number') {
                        display = dtApplyFormat(raw, fmt);
                ...
                // Apply column meta styles
                var meta = colMeta[c] || {};
                var ws = meta.wrap ? 'white-space:normal;' : 'white-space:nowrap;';
                var wStyle = meta.width_style || '';
                td.setAttribute('style', ws + wStyle);
                // Freeze column styling
                if (freezeSet.indexOf(cLower) !== -1) {
                    td.setAttribute('data-freeze', 'true');
                }
                // Truncation
                if (display.length > 120) {
                    td.classList.add('dt-trunc');
                    td.setAttribute('data-full', display);
                    td.setAttribute('onclick', 'dtExpand(this)');
                    td.setAttribute('title', 'Click to expand');
                    td.innerHTML = display.substring(0, 120) + '<span style="color:var(--accent-color);font-weight:bold;">\u2026</span>';
                } else {
        ...
        function dtApplyFormat(num, fmt) {
            // Replicate Python format_file logic in JS
            if (fmt.indexOf('%') === 0) {
                // Date format - raw value is likely not a number but a date string
                return String(num);
            }
            // {:,.0f}
            if (fmt === '{:,.0f}') return num.toLocaleString('en-US', {maximumFractionDigits: 0});
            if (fmt === '{:,.1f}') return num.toLocaleString('en-US', {minimumFractionDigits: 1, maximumFractionDigits: 1});
            if (fmt === '{:,.2f}') return num.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2});
            if (fmt === '{:,.3f}') return num.toLocaleString('en-US', {minimumFractionDigits: 3, maximumFractionDigits: 3});
            if (fmt === '{:,.4f}') return num.toLocaleString('en-US', {minimumFractionDigits: 4, maximumFractionDigits: 4});
            if (fmt === '{:.2%}') return (num * 100).toFixed(2) + '%';
            if (fmt === '{:.1%}') return (num * 100).toFixed(1) + '%';
            if (fmt === '{:.0%}') return (num * 100).toFixed(0) + '%';
            if (fmt === '{:.4f}') return num.toFixed(4);
            if (fmt === '{:.2f}') return num.toFixed(2);
            if (fmt === '{:.1f}') return num.toFixed(1);
            
            // Generic pattern matching
            var pctMatch = fmt.match(/\{:.*?(\d+)%\}/);
            if (pctMatch) return (num * 100).toFixed(parseInt(pctMatch[1])) + '%';
            var commaMatch = fmt.match(/\{:,(.*?)(\d+)f\}/);
            if (commaMatch) return num.toLocaleString('en-US', {minimumFractionDigits: parseInt(commaMatch[2]), maximumFractionDigits: parseInt(commaMatch[2])});
            var fltMatch = fmt.match(/\{:.*?(\d+)f\}/);
            if (fltMatch) return num.toFixed(parseInt(fltMatch[1]));
            // Fallback
