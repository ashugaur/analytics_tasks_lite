...
def convert_html_to_offline(
    ...
    if js_libraries is None:
    ...
    elif isinstance(js_libraries, (str, os.PathLike)):
        lib_path = str(js_libraries)
        if os.path.isdir(lib_path):
            # FOLDER MODE: scan folder for .js files, build map from filenames
            for fname in os.listdir(lib_path):
                if fname.lower().endswith(".js"):
                    fpath = os.path.join(lib_path, fname)
                    key = fname.lower()
                    if "echarts" in key:
                        js_lib_map["echarts"] = fpath
                    elif "d3-sankey" in key or "d3.sankey" in key:
                        js_lib_map["d3-sankey"] = fpath
                    elif "d3" in key and "sankey" not in key:
                        js_lib_map["d3"] = fpath
                    else:
                        norm_key = fname.replace(".min.js", "").replace(".js", "").lower()
                        js_lib_map[norm_key] = fpath
            print(f"    JS folder: {lib_path}")
            print(f"    Available libraries: {list(js_lib_map.keys())}")
        else:
        ...
        for lib_path in js_libraries:
            lib_name = os.path.basename(str(lib_path)).lower()
    ...
    # Find external script tags (with src attribute pointing to http)
    external_scripts = re.findall(
        r'<script[^>]+src\s*=\s*["\'](https?://[^"\'>]+)["\'][^>]*>.*?</script>',
        html_content, re.IGNORECASE | re.DOTALL
    )
    print(f"    Found {len(external_scripts)} external script tag(s)")
    for i, src_url in enumerate(external_scripts):
        print(f"        External {i + 1}: {src_url}")

    # Known CDN URL patterns for each library type
    cdn_patterns = {
        "echarts": [
        ...
        "d3-sankey": [
            "unpkg.com/d3-sankey",
            "cdn.jsdelivr.net/npm/d3-sankey",
        ],
    }

    replaced_count = 0

    # First, find all external <script src="..."> URLs in the HTML
    external_src_tags = re.findall(
        r'<script[^>]+src\s*=\s*["\'](https?://[^"\'>\s]+)["\'][^>]*>.*?</script>',
        html_content, re.IGNORECASE | re.DOTALL
    )

    # Determine which libraries from js_lib_map are actually needed
    # (i.e., have a matching external CDN link in the HTML)
    libs_needed = {}
    for lib_name, lib_path in js_lib_map.items():
        patterns = cdn_patterns.get(lib_name, [lib_name])
        for ext_url in external_src_tags:
            for pattern in patterns:
                if pattern in ext_url:
                    libs_needed[lib_name] = lib_path
                    break
            if lib_name in libs_needed:
                break

    if libs_needed:
        print(f"    Libraries to embed: {list(libs_needed.keys())}")
    else:
        print("    No external CDN links matched available libraries.")

    # NOW do the actual replacement - only for libraries that are needed
    for lib_name, lib_path in libs_needed.items():
        ...
        # Get CDN patterns for this library
        patterns = cdn_patterns.get(lib_name, [lib_name])

        # Search for the <script src="..."> tag matching a CDN pattern
        found = False
        for pattern in patterns:
            idx = html_content.find(pattern)
            if idx != -1:
                start = html_content.rfind("<script", max(0, idx - 300), idx)
                end = html_content.find("</script>", idx) + len("</script>")

                if start != -1 and end != -1:
                    exact_tag = html_content[start:end]
                    print(f"\n🔄 Replacing {lib_name} tag...")
                    print(f"    Removing: {exact_tag[:120]}...")
        ...
        if not found:
            if f"Embedded {lib_name.upper()}" in html_content:
                print(f"\nℹ️  {lib_name} appears already embedded. Skipping.")
            else:
                print(f"❌  CDN URL not found for {lib_name} replacement")
    ...
    if add_browser_check:
        # Add browser verification if requested
        if add_browser_check:
            # Build verification checks only for actually embedded libraries
            lib_checks = []
            # Determine which libs to verify: embedded by us, or detected in HTML
            libs_to_verify = set(libs_needed.keys())
            if not libs_to_verify:
                # Nothing was replaced - check what's actually in the HTML already
                if "echarts" in html_content.lower():
                    libs_to_verify.add("echarts")
                if "d3.sankey" in html_content or "d3-sankey" in html_content.lower():
                    libs_to_verify.add("d3-sankey")
                if "var d3" in html_content or "const d3" in html_content or '"d3"' in html_content:
                    libs_to_verify.add("d3")

            for lib_name in libs_to_verify:
                if lib_name == "echarts":
                    lib_checks.append(
                    ...
                    lib_checks.append(
                        ...
                        "        console.log('%c\u2705 D3 loaded successfully', 'color: #4CAF50;');\n"
                        "    } else {\n"
                        "        console.error('%c\u274c D3 NOT loaded', 'color: #f44336;');\n"
                        "    }"
                    )
                elif lib_name == "d3-sankey":
                    lib_checks.append(
                        "    if (typeof d3 !== 'undefined' && typeof d3.sankey !== 'undefined') {\n"
                        "        console.log('%c\u2705 D3-Sankey loaded successfully', 'color: #4CAF50;');\n"
                        "    } else {\n"
                        "        console.error('%c\u274c D3-Sankey NOT loaded', 'color: #f44336;');\n"
                        "    }"
                    )

        checks_js = "\n".join(lib_checks) if lib_checks else ""
        verification_script = """
...
(function() {{
    ...
    window.addEventListener('load', function() {{
        console.log('%c\U0001f50d Checking if libraries loaded...', 'color: $4CAF50;');
{checks_js}
    }});
}})();
</script>
"""
    ...
    if verify:
        # Only flag external references in script/link/img/iframe tags (not <a> hyperlinks)
        ...
        if external_deps:
            print(
                f"\n⚠️  Warning: HTML still contains {len(external_deps)} external dependency(ies):"
            )
            for ref in external_deps[:5]:
                print(f" - {ref}")
            if len(external_deps) > 5:
                print(f" ... and {len(external_deps) - 5} more")
        else:
            print("\n✅ Fully offline HTML saved")
        # Informational: note any hyperlinks (these are fine)
        hyperlinks = re.findall(
            r'<a[^>]*\shref\s*=\s*"(https?://[^"]+)"', html_content, re.IGNORECASE
        )
        if hyperlinks:
            print(f" ℹ️  {len(hyperlinks)} external hyperlink(s) present (OK - not dependencies)")
...
def create_protected_offline_html(
    ...
    try:
        ...
        if obfuscate:
            ...
            # Skip obfuscation – just copy the offline file to output
            import shutil
            shutil.copy2(temp_offline, str(output_html))
            os.unlink(temp_offline)
            print("\n" + "=" * 70)
            print("✅  COMPLETE! File is now offline (obfuscation skipped).")
            print("=" * 70)
