...
"""
                    ...
                    if _rows:
                        ...
                        for _r in _rows[1:]:
                            ...
                            if _scope == "override" and _tbl_pat:
                                override_column_formats.setdefault((_tbl_pat, _col_pat), _fmt_str)
                                _loaded_o += 1
                            else:
                                global_column_formats.setdefault(_col_pat, _fmt_str)
                                _loaded_g += 1
    ...
    def df_to_html_table(
        ...
        if embed_full_data and full_data_json:
            ...
            # Build column format map for JS-side formatting
            global_formats_lower = {k.lower(): v for k, v in global_column_formats.items()}
            override_formats_lower = {
                (k[0].lower(), k[1].lower()): v for k, v in override_column_formats.items()
            }
            ...
            for col in df.columns:
                ...
                override_key = (table_name.lower(), col_lower)
                if override_key in override_formats_lower:
                    col_formats[col_lower] = override_formats_lower[override_key]
                elif col_lower in global_formats_lower:
                    col_formats[col_lower] = global_formats_lower[col_lower]
            import json as _json_fmt
            formats_js = _json_fmt.dumps(col_formats)
            ...
            window.fullDataFormats_{query_id} = {formats_js}
    ...
    <style>
        *{{margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .nav-link {{
            ...
            font-size: {(nav_font_sizes or {}).get(2, '15px')};
            font-weight: 700;
            flex: 1;
            min-width: 0;
        }}
        ...
        .sidebar-collapsed-strip > * {{ pointer-events: auto; }}
        ...
        .sidebar-contents-badge:hover {{ opacity: 1; background: var(--hover-color); }}
        ...
        .sidebar-nav-dot:hover {{ background: var(--accent-color); transform: scale(1.4); }}

        .sidebar-nav-dot.active {{ background: var(--accent-color); transform: scale(1.3); }}
        ...
        .sidebar-nav-dot:hover .sidebar-nav-dot-tooltip {{ display: block; }}

        .sidebar-backdrop {{ display: none; position: fixed; inset: 0; z-index: 999; }}
    ...
    <div class="sidebar{' collapsible' if sidebar_collapsible else ''}">
        ...
        for section_type, section_data in query_results.items():
            ...
            section_id = section_type.replace(" ", "_").lower()
            html += f"""<div class="result-section" id="section-{section_id}" data-section-id="section-{section_id}">
            <h2 class="result-section-title">{section_type}</h2>"""
    ...
    <script>
        ...
        function filterRowsFullData(queryId, searchTerm, table, countDisplay, fullData, columns, enableMultiWord) {{
            ...
            const colFormats = window['fullDataFormats_' + queryId] || {{}};

            function formatCellValue(value, col) {{
                if (value === null || value === undefined || value === '') return '';
                const fmt = colFormats[col.toLowerCase()];
                ...
                // Generic fallback: try to match {{:.*f}} or {{:.*%}}
                const pctMatch = fmt.match(/\{{:\.\*?(\d+)%\}}/);
                if (pctMatch) return (num * 100).toFixed(parseInt(pctMatch[1])) + '%';
                const fltMatch = fmt.match(/\{{:\.\*?(\d+)f\}}/);
                if (fltMatch) return num.toFixed(parseInt(fltMatch[1]));
                const commaMatch = fmt.match(/\{{:,(.*?)(\d+)f\}}/);
            ...
            if (searchTerm === '') {{
                ...
                for (let i = 0; i < storedRows; i++) {{
                    ...
                    columns.forEach(col => {{
                        ...
                        let cellText = formatCellValue(cellValue, col);
                        if (cellText.length > {{max_cell_chars}}) {{
                            td.classList.add('cell-truncated');
                            td.setAttribute('data-full-text', cellText);
                            td.setAttribute('onclick', 'expandCell(this)');
                            td.setAttribute('title', 'Click to expand');
                            td.innerHTML = cellText.substring(0, {{max_cell_chars}}) + '<span class="truncation-indicator">...</span>';
                        }} else {{
