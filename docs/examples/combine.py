...
def remap_container_access(js: str, slot_index: int) -> str:
    ...
    """
    cell_expr = f"_gridRoot.querySelectorAll('.chart-cell')[{slot_index}]"
    patterns = [
...
def extract_and_remove_set_chart_theme(js: str) -> tuple[str, str | None]:
    ...
    inner_fn = re.sub(
        r"window\.setChartTheme", "__THEME_PLACEHOLDER__", full_block, count=1
    )
    remaining = js[: match.start()] + inner_fn + js[end:]
    return remaining, inner_fn


def is_static_panel(fpath: Path, html: str) -> bool:
    """
    Return True if the file should be treated as a static HTML panel
    rather than an ECharts chart.

    Triggers:
      - Extension is .htmltable, .htmlpanel, .htmldiv, or .fragment
      - OR the file has no echarts.init() call in any inline <script>
    """
    static_extensions = {".htmltable", ".htmlpanel", ".htmldiv", ".fragment"}
    if fpath.suffix.lower() in static_extensions:
        return True
    # Fallback: treat as static if no echarts.init found
    inline = extract_inline_script(html)
    return not re.search(r"\becharts\.init\s*\(", inline)


def extract_panel_html(html: str) -> str:
    """
    Extract the embeddable HTML from a static panel file.

    If the file already looks like a bare fragment (no <html> tag), return as-is.
    Otherwise extract just the <body> contents.
    """
...
def combine_charts(
    ...
    """
    Combine ``cols * rows`` ECharts HTML files (or static HTML panels) into a
    single matrix HTML file compatible with the convert_markdown_to_html.py automation.

    Parameters
    ----------
    input_files : list[Path]
        Input files ordered left-to-right, top-to-bottom.
        Must contain exactly ``cols * rows`` entries.
        Each file is either:
          • An ECharts chart HTML file (.html)  — processed as before.
          • A static HTML panel (.htmltable, .htmlpanel, .htmldiv, .fragment,
            or any .html without an echarts.init() call)  — its HTML content
            is injected directly into the grid cell.  No JS processing is done.
            Use this to place text write-ups, tables, or annotation divs
            alongside charts, combined with col_spacing to control proportions.

        Example — chart on right, write-up panel on left::

            combine_charts(
                input_files=[
                    Path("writeup.htmltable"),        # static panel
                    Path("snippet_histogram.html"),   # ECharts chart
                ],
                cols=2, rows=1,
    ...
    col_spacing : list[float] or None
        Fractional widths for each column, e.g. [0.30, 0.70].
    ...
    for slot_idx, fpath in enumerate(input_files):
        ...
        # ── Static HTML panel (htmltable / htmlpanel / no echarts.init) ──────
        if is_static_panel(fpath, html):
            panel_html = extract_panel_html(html)
            charts.append(
                {
                    "ns": ns,
                    "js": None,               # no JS to run
                    "theme_fn": None,
                    "title": fpath.stem,
                    "slot": slot_idx,
                    "defer": False,
                    "static_html": panel_html,  # inject directly into cell
                }
            )
            continue
        ...
        if theme_fn:
            ...
            named = f"__{ns}_setChartTheme__"
            theme_fn = theme_fn.replace("__THEME_PLACEHOLDER__", named)
            raw_js = raw_js.replace("__THEME_PLACEHOLDER__", named)

        # Hoist theme-related vars out of IIFE so setChartTheme (outer scope)
        # can access them.  'var X = ...' inside an IIFE is local to it;
        # we strip the 'var' so the assignment targets the pre-declared outer var.
        ns_theme_colors = f"{ns}__themeColors"
        ns_theme_ui     = f"{ns}__themeUI"
        raw_js = re.sub(
            r"\bvar\s+" + re.escape(ns_theme_colors) + r"\b",
            ns_theme_colors,   # drop 'var' → writes to outer-scope variable
            raw_js,
        )
        raw_js = re.sub(
            r"\bvar\s+" + re.escape(ns_theme_ui) + r"\b",
            ns_theme_ui,
            raw_js,
        )
        raw_js = re.sub(
            r"\bvar\s+" + re.escape(safe_opt) + r"\b",
            safe_opt,
            raw_js,
        )

        hoisted_extra_vars: list[str] = []
        if theme_fn:
            theme_refs = set(re.findall(r"\b(" + re.escape(ns) + r"_[a-zA-Z_][a-zA-Z0-9_]*)\b", theme_fn))

            already_hoisted = {ns_theme_colors, ns_theme_ui, safe_opt, safe_chart}
            for ref in theme_refs - already_hoisted:
                pattern = r"\bvar\s+" + re.escape(ref) + r"\b"
                if re.search(pattern, raw_js):
                    raw_js = re.sub(pattern, ref, raw_js)
                    hoisted_extra_vars.append(ref)
        ...
        uses_dimensions = bool(re.search(r"\bgetWidth\b|\bgetHeight\b", raw_js))
        ...
        charts.append(
            {
                ...
                "static_html": None,         # not a static panel
                "hoisted_extra": hoisted_extra_vars
            }
        )
    ...
    if col_spacing is not None:
        if len(col_spacing) != cols:
            raise ValueError(
                f"col_spacing has {len(col_spacing)} entries but cols={cols}"
            )
    ...
    if row_spacing is not None:
        if len(row_spacing) != rows:
            raise ValueError(
                f"row_spacing has {len(row_spacing)} entries but rows={rows}"
            )
    ...
    # Grid cells: static panels get their HTML inlined; chart cells stay empty
    cell_parts = []
    for c in charts:
        if c["static_html"] is not None:
            cell_parts.append(
                f'        <div class="chart-cell panel-cell">\n'
                f'{c["static_html"]}\n'
                f'        </div>'
            )
        else:
            cell_parts.append('        <div class="chart-cell"></div>')
    cells_html = "\n".join(cell_parts)

    # Build per-chart IIFE blocks (skip static panels — no JS needed)
    iife_blocks = []
    for c in charts:
        if c["static_html"] is not None:
            continue   # static panel: HTML already inlined, no JS
    ...
    # Pre-declare per-chart theme vars in outer scope (IIFEs assign into these).
    # Only needed for ECharts slots (static panels have no JS).
    def _build_outer_decl(i, c):
        base = f"   var _opt{i} = null, c{i}__themeColors = {{}}, c{i}__themeUI = {{}};"
        extras = c.get("hoisted_extra", [])
        if extras:
            extra_decls = ", ".join(f"{v} = null" for v in sorted(extras))
            base += f"\n    var{extra_decls};"
        return base
    
    outer_decls = "\n".join(
        _build_outer_decl(i, c)
        for i, c in enumerate(charts)
        if c["static_html"] is None
    )
    chart_scripts = "\n".join(iife_blocks)

    # Build JS assignments for static panel content (survives report embedding)
    import json as json
    panel_js_lines = []
    for c in charts:
        if c["static_html"] is not None:
            escaped = json.dumps(c["static_html"])
            panel_js_lines.append(f"    _panelContent[{c['slot']}] = {escaped};")
    panel_js_assignments = "\n".join(panel_js_lines) if panel_js_lines else ""
    ...
    for c in charts:
        if c["theme_fn"]:
            ...
            theme_bodies.append(
                f"        // Theme for {ns}\n"
                f"        try {{\n"
                f"            if (typeof {named} === 'function) {named}(isDark);\n"
                f"        }} catch(e) {{ console.warn('setChartTheme error ({ns})', e); }}"
            )
...
{cells_html}
    ...
    // — Static panel HTML content (injected via JS to survive report embedding) —
    var _panelContent = {{}};
{panel_js_assignments}
    ...
    if (_gridRoot && !_gridRoot.querySelector('.chart-cell')) {{
        ...
        for (var _ci = 0; _ci < {n}; _ci++) {{
            ...
            if (_panelContent[_ci] !== undefined) {{
                _cell.className = 'chart-cell panel-cell';
                _cell.style.overflowY = 'auto';
                _cell.style.fontFamily = 'inherit';
                _cell.innerHTML = _panelContent[_ci];
            }}
            _gridRoot.appendChild(_cell);
        }}
    }} else {{
        // Standalone mode: if panel cells exist but are empty (e.g. after
        // a report system stripped body HTML), re-inject static content.
        var _existingCells = _gridRoot.querySelectorAll('.chart-cell');
        for (var _pi in _panelContent) {{
            if (_panelContent.hasOwnProperty(_pi)) {{
                var _idx = parseInt(_pi, 10);
                if (_existingCells[_idx] && _existingCells[_idx].innerHTML.trim() === '') {{
                    _existingCells[_idx].className = 'chart-cell panel-cell';
                    _existingCells[_idx].style.overflowY = 'auto';
                    _existingCells[_idx].style.fontFamily = 'inherit';
                    _existingCells[_idx].innerHTML = _panelContent[_idx];
                }}
            }}
        }}
    }}
    ...
    // — Per-chart outer-scope vars (themeColors, themeUI, option) —————————
{outer_decls}
...
examples:
  ...
  python combine_charts.py --cols 2 --rows 2 a.html b.html c.html d.html -o combined.html
    ...
    parser.add_argument(
        "--cols", type=int, default=2, help="Number of columns (default: 2)"
    )
    parser.add_argument(
        "--rows", type=int, default=1, help="Number of rows (default: 1)"
    )
    parser.add_argument(
        "-o",
        "--output",
        default="combined_charts.html",
